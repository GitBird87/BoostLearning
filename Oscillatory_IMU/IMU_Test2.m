...
%datashift('base1_data.txt', 1 / 400, 'LoggedData/data2.xlsx','base1_data');%以字符形式打开文件
%datashift('base1.txt',      1 / 400, 'LoggedData/data2.xlsx','base1');     %以字符形式打开文件
...
%% 数据的导入
exp_data     = xlsread('LoggedData/data0.xlsx','base1_data');
base_data    = xlsread('LoggedData/data0.xlsx','base1');

% 数据的清零操作 去除偏移的干扰
gyr      = exp_data(:,  5:7) / 15; acc      = exp_data(:,  2:4) / 1000;
gyr_base = base_data(:, 5:7) / 15; acc_base = base_data(:, 2:4) / 1000;
time     = exp_data(:, 1); samplePeriod = 1/100;
...
%% 静态位置的寻找
stationary(:, 1) = zeros_amend(abs(tcAcc(:, 1)),     0.05, 100);
stationary(:, 2) = zeros_amend(abs(tcAcc(:, 2)),     0.05, 100);
stationary(:, 3) = zeros_amend(abs(tcAcc(:, 3) - 1), 0.05, 100);

tcAcc  = tcAcc - [zeros(length(time), 2) ones(length(time), 1)];  tcAcc  = tcAcc * 9.81;
...
%% 进行位移的积分
linPos(:, 1) = integral_method(linVel(:, 1), samplePeriod, 3);
linPos(:, 2) = integral_method(linVel(:, 2), samplePeriod, 3);
linPos(:, 3) = integral_method(linVel(:, 3), samplePeriod, 3);
